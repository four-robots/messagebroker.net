    ...
    public static async Task RunAsync()
    {
        ...
        try
        {
            ...
            if (File.Exists(profilesJsonPath))
            {
                ...
                Console.WriteLine($"   ✓ History per key: {status.Info.Config.MaxMsgsPerSubject}");
            ...
            if (Directory.Exists(configsPath))
            {
                ...
                foreach (var store in stores)
                {
                    ...
                    Console.WriteLine($"     History: {status.Info.Config.MaxMsgsPerSubject}, Values: {status.Info.State.NumSubjects}");
                }
                Console.WriteLine();
            }
            ...
            // Create KV store
            var kvContext = new NATS.Client.KeyValueStore.NatsKVContext(context.JetStream);
            ...
            Console.WriteLine($"   ✓ History per key: {fullStatus.Info.Config.MaxMsgsPerSubject}");
            ...
            foreach (var bucketName in buckets)
            {
                ...
                Console.WriteLine($"     Values: {status.Info.State.Messages}, Bytes: {status.Info.State.Bytes}");
            }
            Console.WriteLine();
            ...
            Console.WriteLine($"   ✓ Total values: {sessionStatus.Info.State.NumSubjects}\n");
            ...
            var timeoutEntry = await fullStore.GetEntryAsync<string>("app.timeout");
