...
public class KeyValueManagementExample
{
    public static async Task RunAsync()
    {
        ...
        try
        {
            ...
            Console.WriteLine($"   ✓ Values: {status.Info.State.NumSubjects}");
            Console.WriteLine($"   ✓ History: {status.Info.Config.MaxMsgsPerSubject}");
            Console.WriteLine($"   ✓ Bytes: {status.Info.State.Bytes}\n");
            ...
            var user123 = await userProfiles.GetEntryAsync<string>("user:123");
            Console.WriteLine($"   ✓ Retrieved: {user123.Key} = {user123.Value}");

            var user456 = await userProfiles.GetEntryAsync<string>("user:456");
            ...
            var watchedEntry = await userProfiles.GetEntryAsync<string>("user:789");
            Console.WriteLine($"   ✓ Latest value: {watchedEntry.Key} = {watchedEntry.Value}");
            Console.WriteLine($"   ✓ Revision: {watchedEntry.Revision}\n");

            // This functionality is not implemented and thus commented out
