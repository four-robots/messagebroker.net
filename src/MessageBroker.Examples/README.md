# MessageBroker.NET Examples

This project contains comprehensive, runnable examples demonstrating all key features of MessageBroker.NET, with a focus on the enhanced runtime reconfiguration capabilities compared to the original nats-csharp implementation.
...
Unlike nats-csharp which requires server restarts, MessageBroker.NET allows:
...
2. Review the actual implementation in `MessageBroker.Core`
3. Try implementing a real broker controller for NATS
4. Integrate MessageBroker.NET into your own applications

## Requirements

- .NET 9.0 or later
- MessageBroker.Core library
- MessageBroker.Nats library (for actual NATS integration)

## License

This example project is part of MessageBroker.NET and follows the same license as the main project.
